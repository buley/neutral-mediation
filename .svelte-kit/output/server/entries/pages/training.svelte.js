...
const Training = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  $$result.css.add(css);
  return `${$$result.head += `${$$result.title = `<title>Training</title>`, ""}<meta name="${"description"}" content="${""}" data-svelte="svelte-1d88x1l">`, ""}

<div class="${"content svelte-cf77e8"}"><h1>Training</h1>

	<h2>Our Practice</h2>

	<p>Neutral Mediation offers flexibly-scheduled virtual training and practice for people and organizations, led by a professional neutral. <a href="${"/background"}">Read about our neutral here</a>. Optionally, training sessions can be held on-premise at an organization or other agreed-upon location, under the <a href="${"/prices"}">travel policy</a>. <em>Training sessions start at $1,250 per day per participant.</em> For more information about lunch stipends for full-day training sessions, see our <a href="${"/prices"}">pricing page</a>. For individual-focused coaching sessions, see our <a href="${"/coaching"}">coaching page</a>. For neutral-led mediation sessions, see our <a href="${"/mediation"}">mediation page</a>. For neutral-led negotiation sessions, see our <a href="${"/negotiation"}">negotiation page</a>. For mediation, negotiation and conflict consulting for organizations, see our <a href="${"/consulting"}">consulting page</a>.
	</p>

	<h2>Our Value</h2>

	<p>A trained conflict resolution professional helps minimize the trauma, time, and financial expense of complex disputes. By training to recognize how to effectively manage conflict, either informally in negotiation or more formally in mediation processes, trainees are empowered to take the most efficient path to problem-solving in their own lives and teach others to do the same. With confidence in peacemaking skills comes improved well-being.
	</p>
