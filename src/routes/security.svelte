...
<div class="content">
	...
	<p>Neutral Mediation uses Google to encrypt its email and video communications. We mitigate the possibility of email spoofing by using DKIM to certify correspondence. We authenticate our domain using SPF and DMARC records, too, so when an email appears to come from neutrals.io there are multiple ways to verify its provenance. </p>
	<p>Upon request and with collaboration between neutral and participant, any non-streaming correspondence can be encrypted using GPG. The public key <a href="https://buley.wpcomstaging.com/key/">is published online</a>.</p>
</div>
